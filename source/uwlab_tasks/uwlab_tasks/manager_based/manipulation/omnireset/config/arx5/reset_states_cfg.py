...
"""OmniReset reset states configuration for ARX5.

Adapted from ur5e_robotiq_2f85/reset_states_cfg.py:
    - Robot: IMPLICIT_ARX5
    - EE body for IK: link6
    - Gripper joints: joint[78]
    - Arm joints for IK: joint[1-6]
    - No ur5_metal_support (ARX5 uses different mounting)
"""
    ...
    reset_end_effector_pose = EventTerm(
        ...
        params={
            ...
            "robot_ik_cfg": SceneEntityCfg(
                "robot", joint_names=["joint[1-6]"], body_names="link6"
            ),
        },
    )
...
class ObjectRestingEEGraspedEventCfg(ResetStatesBaseEventCfg):
    ...
    reset_end_effector_pose_from_grasp_dataset = EventTerm(
        ...
        params={
            ...
            "gripper_cfg": SceneEntityCfg("robot", joint_names=["joint[78]"]),
            "pose_range_b": {
                "x": (-0.02, 0.02),
                "y": (-0.02, 0.02),
                "z": (-0.02, 0.02),
...
class ResetStatesTerminationCfg:
    ...
    success = DoneTerm(
        ...
        params={
            ...
            "ee_body_name": "link6",
            # ARX5's MuJoCo-converted USD may lack mesh colliders on robot bodies,
            # so we sample points from the objects (which always have mesh) and
            # check distance to the robot as obstacle.
...
@configclass
class Arx5ResetStatesCfg(ManagerBasedRLEnvCfg):
...
@configclass
class ObjectAnywhereEEAnywhereResetStatesCfg(Arx5ResetStatesCfg):
...
@configclass
class ObjectRestingEEGraspedResetStatesCfg(Arx5ResetStatesCfg):
...
@configclass
class ObjectAnywhereEEGraspedResetStatesCfg(Arx5ResetStatesCfg):
...
@configclass
class ObjectPartiallyAssembledEEAnywhereResetStatesCfg(Arx5ResetStatesCfg):
...
@configclass
class ObjectPartiallyAssembledEEGraspedResetStatesCfg(Arx5ResetStatesCfg):
