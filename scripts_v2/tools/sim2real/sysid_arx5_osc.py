...
"""
System Identification for ARX5 using CMA-ES (Closed-Loop Replay).

Uses the manager-based env (OmniReset-Arx5-Sysid-v0) so the same
RelCartesianOSCAction as RL is used — no duplicate OSC.

Parameters (25 total): armature*6, static_friction*6, dynamic_ratio*6,
  viscous_friction*6, motor_delay*1.

The "real data" can come from either the physical ARX5 or from MuJoCo
(sim2sim alignment).  The .pt format is identical:

    {
        "joint_positions":      (T, 6),   # joint angles [rad]
        "initial_joint_pos":    (6,),     # starting config
        "waypoint_step_indices": (W,),    # time steps where target changes
        "waypoint_target_pos":   (W, 3),  # EE target pos in base frame [m]
        "waypoint_target_quat":  (W, 4),  # EE target quat (w,x,y,z)
        "dt":                   float,    # 0.002 (500 Hz)
    }

Usage:
    python scripts_v2/tools/sim2real/sysid_arx5_osc.py --headless \
        --num_envs 512 \
        --real_data /tmp/sysid_data_mujoco.pt \
        --max_iter 200
"""
...
parser = argparse.ArgumentParser(description="ARX5 System Identification via CMA-ES")
parser.add_argument("--num_envs", type=int, default=512)
parser.add_argument("--real_data", type=str, required=True)
parser.add_argument("--max_iter", type=int, default=200)
parser.add_argument("--sigma", type=float, default=0.3)
parser.add_argument("--output_dir", type=str, default="logs/sysid_arx5")
...
from isaaclab.actuators import DelayedPDActuatorCfg  # noqa: E402
from isaaclab.assets import Articulation  # noqa: E402
from isaaclab.utils.math import subtract_frame_transforms  # noqa: E402

from uwlab_assets.robots.arx5.arx5 import ARX5_EFFORT_LIMITS, ARX5_VELOCITY_LIMITS  # noqa: E402
from uwlab_assets.robots.arx5.kinematics import ARM_JOINT_NAMES, EE_BODY_NAME, NUM_ARM_JOINTS  # noqa: E402

import uwlab_tasks  # noqa: F401, E402  # register gym envs
from uwlab_tasks.manager_based.manipulation.omnireset.config.arx5.sysid_cfg import SysidEnvCfg  # noqa: E402
from uwlab_tasks.manager_based.manipulation.omnireset.mdp.utils import settle_robot, target_pose_to_action  # noqa: E402
...
def main():
    ...
    print("\n" + "=" * 60)
    print("ARX5 System Identification - CMA-ES (Closed-Loop Replay)")
    print("=" * 60)
    ...
    # Load reference data (from MuJoCo or real robot)
    ...
    # Override arm actuator with DelayedPDActuator for sysid
    ...
    env = gym.make("OmniReset-Arx5-Sysid-v0", cfg=env_cfg)
    env.reset()
    ...
    print(f"\n  {'Joint':<10s} {'Arm':>8s} {'SFric':>8s} {'DRat':>8s} {'DFric':>8s} {'VFric':>8s}")
    for i, name in enumerate(ARM_JOINT_NAMES):
        print(f"  {name:<10s} {arm[i]:8.4f} {sfric[i]:8.4f} {dratio[i]:8.4f} {dfric[i]:8.4f} {vfric[i]:8.4f}")
